...
1. **Per-task verification (always, author self-check):** After implementing each task, verify its "Done when" criteria before presenting for commit. This is mandatory and automatic. The implementer runs it.
...
**Diff range**: [suggested commit range or branch..HEAD]
