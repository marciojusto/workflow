...
3. User commits each task manually. Never batch tasks; never weaken, skip, or delete tests to make them pass.
...
**One worker per phase:** Each phase worker executes all its tasks in order (implement → gate → stage for user commit), then reports a compact summary (tasks done, commit hashes, test counts, deviations). Workers never spawn further sub-agents.
...
**Standalone fallback:** Without sub-agents, run `validate.md` as an independent fresh-eyes pass after implementation ends — including the spec-anchored check and discrimination sensor.
