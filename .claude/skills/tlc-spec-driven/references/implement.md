...
**Goal**: Implement ONE task at a time. Surgical changes. Verify. Present for commit by the user. Repeat.

This is where code gets written. Every task follows the same cycle: plan → implement → verify → present for commit. Verification is built into every task, not a separate phase.
...
stage for user commit) before moving to the next task in the phase. After all tasks in the phase are
...
1. [Step] → files: [list] → verify: [how] → manual suggested commit message: [message]
2. [Step] → files: [list] → verify: [how] → manual suggested commit message: [message]
3. [Step] → files: [list] → verify: [how] → manual suggested commit message: [message]
```
...
- Independently staggable (gets its own atomic git commit)
...
4. **Test Adequacy Review (MANDATORY — hard gate).**
   ...
   A task cannot be marked done until all four checks below pass. Tests must be both **necessary** (every test traces to a requirement) and **sufficient** (every requirement is covered). The scope boundary is the feature spec — do not test beyond it.
   ...
   **Stack-agnostic litmus:** An assertion is shallow if it would still pass under a plausible *wrong* implementation. If so, strengthen it before presenting for commit.
   ...
   **Anti-patterns — known verification cheats (treat any of these as an automatic Check failure):**
   ...
   | ------------ | ------------ |
   | Marking done before the gate check passes | Skips the deterministic verifier — the gate is not optional |
...
3. **Replace only that section's body** with the current snapshot (feature, phase/task, completed, in-progress `file:line`, next step, blockers, unpresented for commit files, branch). See [memory.md](memory.md) for the exact format.
