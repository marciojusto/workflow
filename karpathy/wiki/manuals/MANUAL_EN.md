...
>  Version: 2.4 | Last Updated: 2026-05-19
...
| **Fallback Model** | kilo/minimax/minimax-m2.7 |
| **Timeout** | 10 minutes |
...
| Deep domain analysis (default) | @miles-expert | kilo/minimax/minimax-m2.7 | 2 | 10min |
| Deep domain analysis (escalation) | @miles-expert | kilo/deepseek/deepseek-v4-pro | 2 | 15min |
| Independent plan review | @review-plan | kilo/z-ai/glm-5.1 | 2 | 10min |
| Architecture coherence validation | @coherence-checker | kilo/minimax/minimax-m2.7 | 2 | 8min |
| Test execution | @e2e-runner | kilo/stepfun/step-3.5-flash:free | 2 | 15min |
...
3. **e2e-runner**: kilo/minimax/minimax-m2.7 → openrouter/qwen-3.6-plus (self-fallback)

### Miles-Expert Model Selection
- **Default**: MiniMax M2.7 (204,800 tokens) for low/medium complexity, single-module analysis
...
**@miles-expert:**
```
Analyze Jira ticket MMH-1435 and identify affected APIs
```
...
| Create ticket note | "Create a ticket note for MMH-1435 implementation" |

#### Miles-expert Prompts:

| Example | Prompt |
|---------|--------|
| Analyze ticket | "Analyze Jira ticket MMH-1435: Create quote workflow" |
...
| Test all ACs | "Run all ACs for ticket MMH-1435" |
...
| `hyperfront/.gitnexus/` | hyperfront index |
| `deal-bs/.gitnexus/` | deal-bs index |
...
│   ├── docker-compose.yml
│   ├── scan-hyperfront.sh   # SonarQube for frontend projects (Nuxt/Vue/Node)
│   └── scan-deal-bs.sh      # SonarQube for Java projects (deal-bs)
├── karpathy/            # Wiki system
│   ├── control/          # index.md, log.md
│   ├── history/          # Historical implementation records
│   ├── raw/              # Source documents
│   │   ├── files/        # PDFs, data files (MMH_1435, MMH_1465)
...
| `docker/scan-hyperfront.sh` | Run SonarQube for frontend projects |
| `docker/scan-deal-bs.sh` | Run SonarQube for Java projects |
...
| `step-log.py` not writing | `logs/` dir doesn't exist | `mkdir -p ~/Development/teamwill/mobilize/workflow/logs/` |
| Orchestrator not advancing | Exit gate not passed | Check `step-log.py status <workflow_id>` to see which gate is blocked. |
| Wiki not syncing to Obsidian | sync-obsidian.sh didn't run | Run manually: `bash ~/Development/teamwill/mobilize/workflow/scripts/sync-obsidian.sh` |
...
| Re-index GitNexus | After major changes | `npx gitnexus analyze --force` in hyperfront/ and deal-bs/ |
...
| e2e-runner | kilo/stepfun/step-3.5-flash:free | FREE | 2 | 15min |
...
*Changes v1.8: Automatic code principles verification (DRY, KISS, YAGNI, SOLID, SoC) in review-implementation step. New AGENTS.md for deal-bs.*

*Changes v1.7: Auto-detect project type (frontend/backend Java/backend Node). Dynamic steps for testing and validation.*

*Changes v1.6: Removed Hyperfront project references - workflow is project-agnostic.*
