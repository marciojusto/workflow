...
**@miles-expert:**
```
Analisa o ticket Jira PROJECT-XXXX e identifica as APIs afetadas
```
...
| Criar nota de ticket | "Criar uma nota de ticket para a implementação do PROJECT-XXXX" |

#### Prompts do Miles-expert:

| Exemplo | Prompt |
|---------|--------|
| Analisar ticket | "Analisa o ticket Jira PROJECT-XXXX: Create quote workflow" |
...
| Testar todas ACs | "Executar todas as ACs para o ticket PROJECT-XXXX" |
...
| `frontend-app/.gitnexus/` | Índice do frontend-app |
| `backend-app/.gitnexus/` | Índice do backend-app |
...
│   ├── docker-compose.yml
│   ├── scan-frontend-app.sh   # SonarQube para projetos frontend (Nuxt/Vue/Node)
│   └── scan-backend-app.sh      # SonarQube para projetos Java (backend-app)
├── karpathy/            # Sistema de wiki
│   ├── control/          # index.md, log.md
│   ├── history/          # Registos históricos de implementação
│   ├── raw/              # Documentos fonte
│   │   ├── files/        # PDFs, ficheiros de dados (PROJECT_XXXX, PROJECT_YYYY)
...
| `docker/scan-frontend-app.sh` | Executar SonarQube para projetos frontend |
| `docker/scan-backend-app.sh` | Executar SonarQube para projetos Java |
...
| `step-log.py` não escreve | Diretório `logs/` não existe | `mkdir -p ~/Development/company/project/workflow/logs/` |
...
| Wiki não sincroniza com Obsidian | sync-obsidian.sh não correu | Executar manualmente: `bash ~/Development/company/project/workflow/scripts/sync-obsidian.sh` |
...
| Re-indexar GitNexus | Após mudanças grandes | `npx gitnexus analyze --force` em frontend-app/ e backend-app/ |
...
*Alterações v1.8: Verificação automática de princípios de código (DRY, KISS, YAGNI, SOLID, SoC) no step review-implementation. Novo AGENTS.md para backend-app.*
...
*Alterações v1.6: Removidas referências ao projeto Frontend - workflow independente de projeto.*
